...
const http = axios.create({
  ...
  headers: {
    ...
    'Content-Type': 'application/json',
    'HTTP-Referer': 'https://your-app.local', // 🔥 IMPORTANT
    'X-Title': 'AI-OCR-App'
  }
});
...
async function callChatWithRetry({ model, messages, maxTokens, temperature }) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (status === 429) {
        return { ok: false, fallbackText: 'AI busy. Try later.' };
      }
...
async function generateEmbeddings(texts) {
  ...
  try {
    ...
    if (!Array.isArray(resp.data?.data)) throw new Error('Invalid embedding');

    return resp.data.data.map(d => d.embedding || []);

  } catch (err) {
    console.warn('Embedding fallback:', err.message);
    return inputs.map(() => new Array(32).fill(0));
  }
}
